...
// normal color buffer
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (unsigned int i = 0; i < lightPositions.size(); i++)
        {
            ...
            shaderLightingPass.setVec3("lights[" + std::to_string(i) + "].ambient", pointLight.ambient);
            shaderLightingPass.setVec3("lights[" + std::to_string(i) + "].diffuse", pointLight.diffuse);
            shaderLightingPass.setVec3("lights[" + std::to_string(i) + "].specular", pointLight.specular);
            shaderLightingPass.setFloat("lights[" + std::to_string(i) + "].constant", pointLight.constant);
            shaderLightingPass.setFloat("lights[" + std::to_string(i) + "].linear", pointLight.linear);
            shaderLightingPass.setFloat("lights[" + std::to_string(i) + "].quadratic", pointLight.quadratic);
        }
        ...
        //spotlight
        shaderLightingPass.setVec3("spotLight.position", programState->camera.Position);
        shaderLightingPass.setVec3("spotLight.direction", programState->camera.Front);
        shaderLightingPass.setVec3("spotLight.ambient", 0.0f, 0.0f, 0.0f);
        shaderLightingPass.setVec3("spotLight.diffuse", 1.0f, 1.0f, 1.0f);
        shaderLightingPass.setVec3("spotLight.specular", 1.0f, 1.0f, 1.0f);
        shaderLightingPass.setFloat("spotLight.constant", 1.0f);
        shaderLightingPass.setFloat("spotLight.linear", 0.09);
        shaderLightingPass.setFloat("spotLight.quadratic", 0.032);
        shaderLightingPass.setFloat("spotLight.cutOff", glm::cos(glm::radians(12.5f)));
        shaderLightingPass.setFloat("spotLight.outerCutOff", glm::cos(glm::radians(15.0f)));

        shaderLightingPass.setBool("blinn",true);
        shaderLightingPass.setBool("hdr", programState->hdr);
        shaderLightingPass.setFloat("exposure", programState->exposure);
        std::cout << "hdr: " << (programState->hdr ? "on" : "off") << "| exposure: " << programState->exposure << std::endl;
        // finally render quad
        renderQuad();
        ...
        if (programState->ImGuiEnabled) {
...

            {
                ...
                ImGui::SliderFloat("Float slider", &programState->exposure, 0.0, 5.0);
                ImGui::ColorEdit3("Background color", (float *) &programState->clearColor);
                ImGui::DragFloat3("Backpack position", (float *) &programState->backpackPosition);
                ImGui::DragFloat3("dirLightDiffuse", (float *) &programState->dirLightDiffuse);
                ImGui::DragFloat3("dirLightAmbient", (float *) &programState->dirLightAmbient);
                ImGui::DragFloat3("dirLightSpecular", (float *) &programState->dirLightSpecular);
